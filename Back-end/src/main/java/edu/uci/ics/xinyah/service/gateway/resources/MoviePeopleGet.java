...
@Path("movies/people")
public class MoviePeopleGet {
    @Path("get/{phrase:.*}")
    @GET
    @Produces(MediaType.APPLICATION_JSON)
    public Response people(@Context HttpHeaders headers, @Context UriInfo uriInfo, String jsonText, @PathParam("phrase") String phrase)
    {
        ...
        clientRequest=new ClientRequest(headers.getHeaderString("email"),
                ...
                moviesConfigs.getPeopleGetPath()+phrase,
                HTTPMethod.GET,
                jsonText.getBytes(),
                11,
                queries
