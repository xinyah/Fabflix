...
@Path("movies")
public class MoviePeopleSearch {
    @Path("people/search")
    @GET
    @Produces(MediaType.APPLICATION_JSON)
    public Response peopleSearch(@Context HttpHeaders headers, @Context UriInfo uriInfo, String jsonText)
    {
        ...
        clientRequest=new ClientRequest(headers.getHeaderString("email"),
                ...
                moviesConfigs.getPeopleSearchPath(),
                HTTPMethod.GET,
                jsonText.getBytes(),
                11,
                queries
